...
const ZenToolbar: React.FC<ZenToolbarProps> = ({
    currentTool, onToolChange, onClear, onDownload, brushSize, onSizeChange, wetness, onWetnessChange, brushColor, onColorChange
}) => {
    const [isExpanded, setIsExpanded] = useState(true)
    const [isHidden, setIsHidden] = useState(false)
    const [pos, setPos] = useState({ x: window.innerWidth / 2 - 270, y: window.innerHeight - 150 })
    const [lastInk] = useState<'INK' | 'RED'>('INK')
    ...
    const curColor = currentTool === 'ERA' ? '#666' : brushColor
                ...
                {isExpanded && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }} onTouchStart={e => e.stopPropagation()}>
                        ...
                        <div style={{ position: 'relative', width: '48px', height: '48px', flexShrink: 0 }}>
                            <div style={{ ...btnStyle, position: 'absolute', top: 0, left: 0, pointerEvents: 'none' }}>
                                <PaletteIcon color={brushColor} />
                            </div>
                            <input
                                type="color"
                                value={brushColor}
...
const PaletteIcon = ({ color }: { color: string }) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.647-.494 2.152-1.127a2.49 2.49 0 0 0 .5-1.921c-.139-.773.08-1.577.567-2.115l.08-.088c.451-.497 1.157-.749 1.831-.749h1.12c1.944 0 3.75-1.547 3.75-3.5 0-5.247-4.5-9.5-10-9.5z" stroke="#222" />
